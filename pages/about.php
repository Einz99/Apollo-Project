<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>About Apollo+ Ent</title>
    <script
      src="https://kit.fontawesome.com/d9e96eb9b3.js"
      crossorigin="anonymous"
    ></script>
    <link
      rel="stylesheet"
      href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css"
      integrity="sha512-SnH5WK+bZxgPHs44uWIX+LLJAJ9/2PkPKZ5QiAj6Ta86w+fsb2TkcmfRyVX3pBnMFcV7oQPJkl9QevSCWr3W6A=="
      crossorigin="anonymous"
      referrerpolicy="no-referrer"
    />
    <link rel="stylesheet" href="../css/headerfooter.css">
    <link rel="stylesheet" href="../css/about.css">
    <link rel="icon" type="image" href="../assets/favicon.ico">
</head>
<body> 
    <header>
        <a href="#" class="brand"
          ><img src="../assets/APOLLO+ LOGO VER2.png" class="logo"
        /></a>
        <div class="rightmost">
          <div class="media-icons">
            <a href="https://www.facebook.com/ribnan.sangalang.1?mibextid=ZbWKwL"
              ><i class="fab fa-facebook-f"></i
            ></a>
            <a
              href="https://www.instagram.com/san.rayyy?igsh=MWpwamw0OTNocmViZQ=="
              ><i class="fab fa-instagram"></i
            ></a>
            <a href="https://x.com/positiviSTAY?t=OBzg7zTppq4kQvGMYVfbtQ&s=09"
              ><i class="fa-brands fa-x-twitter"></i
            ></a>
          </div>
          <div class="menu-btn"></div>
          <div class="navigation">
            <div class="navigation-items">
              <a href="../index.php">Home</a>
              <a href="artists/artist.php">Artists</a>
              <a href="artists/shop.php">Shop</a>
              <a href="#"  class="active">About</a>
              <a href="contact/contact.php">Contact</a>
              <div class="reserved">
                <p>©️ <i>2024 Apollo+ Entertainment, Inc. All Rights Reserved.</i></p>
              </div>
          </div>
        </div>
      </header>
      <script type="text/javascript" src="../js/header.js"></script>
    <section>
        <div class="banner">
            <h1>ABOUT</h1>
            <img src="../assets/banner.png">
        </div>
        <br><br><br><br><br><br>
        <div class="banner2">

        </div>
    </section>
    <div class="about">Lorem ipsum dolor sit, amet consectetur adipisicing elit. Optio id porro exercitationem officiis quo qui. Magni veritatis impedit tenetur suscipit facilis recusandae atque, ad, sint veniam distinctio eaque temporibus qui?</div>
    <section class="content">
      <div class="mission">
        <div class="pictures">
          <div class="pic1"></div>
          <div class="pic2"></div>
          <div class="pic3"></div>
        </div>
        <div class="message">
          <H1>MISSION</H1>
          <p>To create unparalleled entertainment experiences that captivate and inspire audiences worldwide, blending creativity, innovation, and excellence in every performance.</p>
        </div>
      </div>
      <div class="vision">
        <div class="message1">
          <H1>VISION</H1>
          <p>To be the leading global entertainment company, setting the standard for quality and creativity, and transforming the way the world experiences live entertainment.</p>
        </div>
        <div class="pictures">
          <div class="pic1"></div>
          <div class="pic2"></div>
          <div class="pic3"></div>
        </div>
      </div>
      <div class="values">
        <div class="pictures">
          <div class="pic1"></div>
          <div class="pic2"></div>
          <div class="pic3"></div>
        </div>
        <div class="message">
          <H1>VALUES</H1>
          <p>Creativity: We embrace innovation and originality in all our productions, striving to push the boundaries of entertainment.<br>
            Excellence: We are committed to delivering high-quality experiences that exceed expectations and leave a lasting impact.<br>
            Integrity: We operate with transparency, honesty, and respect, fostering trust with our audiences, partners, and employees.<br>
            Inclusivity: We celebrate diversity and aim to create inclusive environments where everyone feels valued and represented.<br>
            Passion: We are driven by a love for entertainment and a dedication to bringing joy and wonder to people around the globe.</p>
        </div>
      </div>
    </section>  
    <footer>
        <br><br>
      <div class="footHeader1">
        <div class="foot1">
          <img src="../assets/APOLLO+ LOGO VER2.png" class="logo"/>
        </div>
        </div>
      </div>
      <div class="footcontact">
        <div class="footiii">
          <p class="heading">CONTACT US</p>
          <p>For more info about booking, inquiries and audition</p>
          <a href="pages/contact/contact.html" class="contact"><i>Click here</i></a>
          <br>
          <a href="https://www.facebook.com/ribnan.sangalang.1?mibextid=ZbWKwL"
            ><i class="fab fa-facebook-f"></i
          ></a>
          <a
            href="https://www.instagram.com/san.rayyy?igsh=MWpwamw0OTNocmViZQ=="
            ><i class="fab fa-instagram"></i
          ></a>
          <a href="https://x.com/positiviSTAY?t=OBzg7zTppq4kQvGMYVfbtQ&s=09"
            ><i class="fa-brands fa-x-twitter"></i
          ></a>
        </div>
        <div class="foot4">
            <div class="foot41">
                <p><i class="fa-solid fa-location-dot"></i><br><br></p>
                <br>
                <p><i class="fa-solid fa-phone"></i></p>
                <br>
                <p></p><i class="fa-solid fa-envelope"></i></p>
            </div>
            <div class="foot42">
                <p>MXV9+GJF, Maysan Rd <br>Valenzuela, Metro Manila</p>
                <br>
                <p>6391234567890</p>
                <br>
                <p>apolloentertainment@gmail.com</p>
            </div>
        </div>
      </div>
      <div class="license">
        <p>©️ <i>2024 Apollo+ Entertainment, Inc. All Rights Reserved.</i></p>
      </div>
    </footer>
</body>
</html>